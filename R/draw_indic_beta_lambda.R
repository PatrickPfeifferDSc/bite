#' Draw BETA Indicators and Coefficients
#'
...
draw_indic_beta_lambda <- function(y,X,sgma2,delta,deltafix,omega1,omega2,dy,invB0,isel){
  df <- length(delta)
  nd <- df - sum(deltafix)

  ysy <- t(y/sgma2) %*% y
  XS <- t(X) %*% Diagonal(x = as.vector(1/sgma2))   # pck use 'Matrix' transforms XS into dgCMatrix class
