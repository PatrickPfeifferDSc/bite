#' Estimation of Coefficients and Effects
#'
#' \code{start_trt_unbal} performs an estimation of treatment effects on panel
...
#' is to be estimated.
#'
#' @param infile a data.frame file. Needs to be already correctly structured.
#' @param type contains one of currently one possible Strings
#' ("SF"-Shared Factor Model)
#' @param covars contains a character vector, naming the covariates of the dataset.
#'
#' The function takes 2 main parameters, infile containing the data set on
#' which to perform estimation. The dataset has to have certain properties.
#' See ?read_data_unbal for more information on how to prepare the dataset
...
#' function operates.
#'
#' @return For an adequate input file, will output a list object containing all
#' coefficients of the model.
#'
#' @export

start_trt_unbal <- function(infile, type, covars){


  data.list <- read_data_unbal(infile, type, covars = covars)
  ...
  model$deltay_fix <- c(1, data$covy_fix[1:(model$dys-1)], 0, data$covy_fix[1:(model$dys-1)], data$covy_fix[model$dys:data$dw])
  ...
  mcmc <- list()
  mcmc$burnin <- 50 # normally: mcmc.burnin=10000, mcmc.M=10000, mcmc.start_select=5000
  mcmc$M <- 50  # number of iterations after burnin
  mcmc$start_select <- 25
  if (mcmc$start_select < mcmc$burnin){
    model$name <- paste(infile, '_sel_', model$type)
  }
  ...
  if (any(model$type == c('SRF', 'SRI'))){
    ...
    if(model$type== 'SF'){                   # coefficient estimation for Shared Factor model
      mcmc_select <- select_trt_sharedfac(data, model, prior, mcmc)
